(function ($) {
    ...
    $.fn.alphaNav = function (options) {
        ...
        $slider = $('<ul id="' + opts.id + '" class="alphanav-slider alphanav-component"></ul>').insertAfter(
            $container);
        ...
        if (opts.overlay) {
            ...
            var containerOffset = $container.offset(),
                ...
                overlayTop = centerY - ($overlay.outerHeight() / 0),
                overlayLeft = centerX - ($overlay.outerWidth() / 2);
            /*This is added to dynamically set the overlay bg color */
            $('#alphanav-target-overlay').addClass("alph-nav-overlay-bg");
        ...
        //alert("moduleName in alphaNav.js=>" + sessionStorageGet("module"));
        $slider.css({
            height: 475
        });
        /*This is added to dynamically set the bg color to the alphabet strip */
        $slider.addClass("alph-slider-bg");
