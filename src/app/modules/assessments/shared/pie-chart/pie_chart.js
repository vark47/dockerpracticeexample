var Pizza = {
  ...
  init: function (scope, options) {
    ...
    // window.sessionStorage.setItem("pieName", pieName);
  ...
  events: function () {
    ...
    $(this.scope).off('.pizza').on('mouseenter.pizza mouseleave.pizza touchstart.pizza', '[data-pie-id] li', function (e) {
      ...
      //    console.log("inside pizza events Pizza.pieName:"+Pizza.pieName);
    ...
    if (Pizza.pieName == "svg") {
      el.hover(function (e) {
        var path = Snap(e.target),
          text = Snap($(path.node).parent()
            .find('text[data-id="' + path.node.getAttribute('data-id') + '"]')[0]);
        alert("pie popup" + $(path.node).parents(".main-svg").html());
        path.animate({
  ...
  svg: function (legend, settings) {
    ...
    //var pieName=window.sessionStorage.getItem("pieName");
