var Pizza = {
  ...
  init: function (scope, options) {
    ...
    Pizza.pieName = $('[data-pie-id]').data("pie-id");
  ...
  events: function () {
    ...
    var ref = this;
    $(this.scope).off('.pizza').on('mouseenter.pizza mouseleave.pizza touchstart.pizza touchend.pizza focusin.pizza  focusout.pizza', '[data-pie-id] a', function (e) {
      /*This condition applies for the pie-chart present in the result screen of Interest profiler
       *for the hovering functionality.**/
      // alert('focused in pie_chart');
      try {

        if (Pizza.pieName == "svg") {
          if (e.type + '' == 'mouseenter') {
            $(ref.scope).focusout();
          }
          console.log('pie_chart $(this).index():' + $(this).index() + "(e.type + '' == 'mouseenter'):" + (e.type + '' == 'mouseenter'));
          var parent = '';
          try {

            parent = $(this).parents('[data-pie-id]'),
              path = Snap($('#' + parent.data('pie-id') + ' path[data-id="s' + $(this).data('inx') + '"]')[0]),

              text = Snap($(path.node).parent().find('text[data-id="' + path.node.getAttribute('data-id') + '"]')[0]),
              settings = parent.data('settings');
          } catch (e) {
            console.log('pie_chartparentget exception:' + e.message);
          }
          console.log('pie_chart path.node:' + path.node + 'e.type:' + e.type + '/start/i.test(e.type):' + (/start/i.test(e.type)) + '(/enter|start/i.test(e.type)):' + (/enter|start/i.test(e.type)));// + '$(path.node).siblings("path"):' + $(path.node).siblings('path').html());
          if (/start/i.test(e.type)) {
            $(path.node).siblings('path').each(function () {
              console.log('pie_chart path:' + path + ' this.nodeName:' + this.nodeName + '$(this).next():' + $(this).next());
              if (this.nodeName) {
                path.animate({
          ...
          } else if ((e.type == 'mouseleave') || (e.type == 'touchend') || (e.type == 'focusout')) {
            path.animate({
              transform: 's1 1 ' + path.node.getAttribute('data-cx') + ' ' + path.node.getAttribute('data-cy')
            }, settings.animation_speed, mina[settings.animation_type]);
            text.animate({
        ...
        console.log('pizza exception:' + e.message);
      }
    });

    $(this.scope).on('.Pizza').on('click', '[data-pie-id] li', function (e) {
      /*This condition applies for the pie-chart present in the occlist screen of Interest profiler
      *for the click functionality.**/
      if (Pizza.pieName != "svg") {
        try {
          ...
          alert("pizza chart exception:" + e.message);
        }
      }
    });
  },
  ...
  animate: function (el, cx, cy, settings) {
    ...
    if (Pizza.pieName == "svg") {
      el.hover(function (e) {
        ...
        if ($(path.node).parents(".main-svg").attr("id") + "" == "undefined") {
  ...
  svg: function (legend, settings) {
    ...
    var customWidth, customHeight;
    /**The width and height of the pie-chart are assisgned based on the screen size.
     * The first condition is for the pie-chart present in the ip-occlist.The other 
     * is for the pie-chart present in the ip-result.
     */
    if (Pizza.pieName != "svg") {
      if ($(window).width() < 767) {
        customWidth = 80;
    ...
    svg.node.setAttribute('width', customWidth + settings.percent_offset);
    svg.node.setAttribute('class', "piechartclass");
    svg.node.setAttribute('height', customHeight + settings.percent_offset);
